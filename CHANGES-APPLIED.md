...
- The journey accordion replaced by **The day, in short** — six statement lines, no
  ...
  **Reversed 2026-09-04:** the six lines are accordion rows again. The line you
  already read stays the closed row, and opening it adds the fuller passage —
  taken word for word from the matching step on What to Expect. Still no numbering.
- Questions accordion replaced by three question headings linking through; the
  ...
  **Reversed 2026-09-04:** the three questions are an accordion again, each opening
  onto its approved answer from the questions page. *View more questions* still
  carries through to `faqs.html`.

> The two reversals above answer the 4 September review: both blocks looked
> openable and were not. No new copy was written for either — run
> `node scripts/check-home-copy-provenance.js` to see every sentence traced back
> to `what-to-expect.html` or `faqs.html`.
...
- **Fixed 2026-09-04:** *The two that stay open* did not. Both rows sat in one
  `data-bs-parent` group, so opening either one shut the other — against the
  section's own heading. The two panels are now independent and both stay open.
